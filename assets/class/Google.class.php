<?php
	...
	* Usage:
	*
	* To redirect users to a Google login page, simply create a Google:
	*
	* $googleGateway = Google::createRequest("http://www.mydomain.com/check.php",
	*                                              "ABCDEFG",
	*                                              true);
	*
	...
	* given or it is set to null, Google will automatically request an
	...
	* handle on your own using Google::getAssociationHandle().
	*
	...
	* Example:
	*
	* $googleGateway = Google::createRequest("http://www.mydomain.com/checkauth.php", "ABCDEFG", true);
	* $googleGateway->redirect();
	* 
	* OR
	*
	* $handle = Google::getAssociationHandle(); // <--save this! valid for two weeks!
	*
	* $googleGateway = Google::createRequest("http://www.mydomain.com/checkauth.php", $handle, true);
	* $googleGateway->redirect();
	*
	*
	* When you want to recieve a Google OpenID response, simply pass the given
	* parameters to Google::create(). In most cases, you should just be able
	* to pass the $_GET variable.
	*
	* To continue the previous example, the following code would go in checkauth.php
	*
	* $googleResponse = Google::create($_GET);
	...
	* OR, even easier
	*
	* $googleResponse = Google::getResponse(); // <-- automatically reads $_GET
	*
...
class Google {
	...
	//private constructor
	private function Google($mode, $op_endpoint, $response_nonce, $return_to, $realm, $assoc_handle, $claimed_id, $signed, $sig, $email, $require_email){
	
		//if assoc_handle is null, fetch one
		if(is_null($assoc_handle))
		$assoc_handle = Google::getAssociationHandle();
		...
		//if the first character is a slash, delete it
			if(substr($return_to, 0, 1)=="/") $return_to = substr($return_to, 1);
	...
	//this creator should be used when creating a Google for a redirect
	public static function createRequest($return_to, $assoc_handle=null, $require_email=false){
	  return new Google("checkid_setup", null, null, $return_to, null, $assoc_handle, "http://specs.openid.net/auth/2.0/identifier_select", null, null, null, $require_email);
	}//createRequest
	...
	public static function create($params){
	  ...
	  if(is_null($return_to) && $mode=="checkid_setup")
	    throw new Exception("Google.create() needs parameter openid.return_to");
	
	  //return a new Google with the given parameters
	  return new Google($mode, $op_endpoint, $response_nonce, $return_to, $realm, $assoc_handle, $claimed_id, $signed, $sig, $email, $require_email);
	}//create
	
	//creates and returns a Google from the $_GET variable
	public static function getResponse(){
	  return Google::create($_GET);
	}//getResponse
	...
	public static function getAssociationHandle($endpoint=null){
	  ...
	  if(is_null($endpoint))
	    ...
	    $request_url = Google::getEndPoint();
	  //if endpoint given, set it
	  else
	    $request_url = $endpoint;
	  
	  //append parameters (these never change)
	  $request_url .= "?openid.ns=".urlencode(Google::openid_ns);
	...
	public static function getEndPoint(){
	  $request_url = Google::google_discover_url;
	  $c = curl_init($request_url);
	  curl_setopt($c, CURLOPT_RETURNTRANSFER, true);
	  curl_setopt($c, CURLOPT_HEADER, false);
	  $request_contents = curl_exec($c);
	...
	public function getArray(){
	  ...
	  $ret['openid.ns'] = Google::openid_ns;
	  ...
	  if(($this->mode=="checkid_setup" AND $this->require_email) OR
	    ...
	    $ret['openid.ns.ext1'] = Google::openid_ns_ext1;
	    $ret['openid.ext1.mode'] = Google::openid_ext1_mode;
	    $ret['openid.ext1.type.email'] = Google::openid_ext1_type_email;
	    $ret['openid.ext1.required'] = Google::openid_ext1_required;
