...
class MapTable {
	...
	function __construct($db, $table_name = null, $parameters = array(), $sticky_fields = array()){
		...
		if(self::checkValues() and $this->primary_value == false and !isset($_POST['this_is_the_primary_value']) and !isset($_GET['ajax']) and !empty($_POST)){ self::insertRecord(); self::whereDoWeGoNext(); }
		
		//if form submitted
		if(self::checkValues() and $this->primary_value > 0 and isset($_POST['this_is_the_primary_value']) and $_POST['this_is_the_primary_value'] == $this->primary_value and !isset($_GET['ajax'])){ self::updateRecord(); }

		//delete row
		if(isset($_POST['form_submit']) and $_POST['form_submit'] == 'Delete Record' and !isset($_GET['ajax'])){ self::deleteRecord(); }
	...
	protected function changeActiveSettingForRecord($record_id, $active_value){
		...
		if(mysql_affected_rows() > 0){ return json_encode(array('id' => (int)$record_id, 'active' => $active_value)); }
		return null;
	}
	...
	protected function specialField($key, $val, $mainTableCheck = true){
		...
		switch(true){
			...
			case(substr($val, -5) == 'state'):
				...
				if($mainTableCheck and isset($this->states)){ $return_array['states_array'] = $this->states; }
			break;
	...
	protected function checkInputField($field){
		if(isset($field) and !empty($field)){
			switch(true){
				...
				//int
				case(substr($field['Type'], 0, 3) == 'int' and (!isset($field['field_type']) or (isset($field['field_type']) and $field['field_type'] != 'orderfield'))):
					return $field;
				break;
	...
	/*********************** HANDLE FIELD LABEL ******************/
	/*************************************************************/	
	public function handleFieldLabel($field = null){
		if(!have($field['field_name'])){ return null; }
		$required_text = (isset($field['field_regex']) and $field['field_regex'] == '*' or strlen($field['field_regex']) > 1) ? ' <span title="required">*</span>' : null;
		return ucwords(preg_replace(array('/\(([^\)]+)\)/'), array('<span class="label_sub_text">$1</span>'), $field['field_name'])) . ':' . $required_text;
	}
