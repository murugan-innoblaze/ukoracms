...
//try form class
$Album = new Album($db, 'dzpro_albums', array('states' => $states_list));

//$Album->showForeignTablesWithPrimaryKey();

//whitelist foreign table with primary key .. input array - get them with $Album->showForeignTablesWithPrimaryKey();
$Album->whitelistForeignTables(array('dzpro_orders'));
?>
<!DOCTYPE html> 
<html lang="en-us"> 
	<head> 
		<?php require RELATIVE_ASSETS_PATH . '/elements/head.element.php'; ?>
		<?php $Album->buildHeadBlock(); ?>
	</head>
	<body>
		<div id="wrapper">
				...
				<tbody>
					<tr>
						...
						<td id="right_column">
							...
							<?php $show_form = $Album->showEventBlock(); ?>
							<table id="inner_content_table" cellpadding="0" cellspacing="0">
								<tbody>
									<tr>
										<td id="inner_content_left">
											<div id="bucket">
												<?php $Album->buildFromToolbar(); ?>
												<?php $Album->buildFromListing(); ?>												
											</div><!-- .bucket -->
