...
//try form class
$News = new News($db, 'dzpro_site_notifications', array(), array());

//$News->showForeignTablesWithPrimaryKey();

//whitelist foreign table with primary key .. input array - get them with $News->showForeignTablesWithPrimaryKey();
//$News->whitelistForeignTables(array('dzpro_comments', 'dzpro_user_meta'));
?>
<!DOCTYPE html> 
<html lang="en-us"> 
	<head> 
		<?php require RELATIVE_ASSETS_PATH . '/elements/head.element.php'; ?>
		<?php $News->buildHeadBlock(); ?>
	</head>
	<body>
		<div id="wrapper">
				...
				<tbody>
					<tr>
						...
						<td id="right_column">
							...
							<?php $show_form = $News->showNewsEventBlock(); ?>
							<table id="inner_content_table" cellpadding="0" cellspacing="0">
								<tbody>
									<tr>
										<td id="inner_content_left">
											<div id="bucket">
												<?php $News->buildFromToolbar(); ?>
												<?php $News->buildFromListing(); ?>												
											</div><!-- .bucket -->
