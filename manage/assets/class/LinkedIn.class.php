...
class LinkedIn {
  ...
  function simpleShare($status){
  	$status_url = $this->base_url . "/v1/people/~/shares";
  	$xml = '<share><comment>' . htmlspecialchars($status, ENT_NOQUOTES, "UTF-8") . '</comment><visibility><code>anyone</code></visibility></share>';
  	$request = OAuthRequest::from_consumer_and_token($this->consumer, $this->access_token, "POST", $status_url);
    $request->sign_request($this->signature_method, $this->consumer, $this->access_token);
    $auth_header = $request->to_header("https://api.linkedin.com");
    $response = $this->httpRequest($profile_url, $auth_header, "POST", array('xml' => $xml));
    return $response;
  }
  ...
  function httpRequest($url, $auth_header, $method, $body = NULL) {
    ...
    if(false === ($data = curl_exec($curl))){ echo curl_error($curl); }
